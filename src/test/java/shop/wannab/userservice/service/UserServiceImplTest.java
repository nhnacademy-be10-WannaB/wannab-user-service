...
//    }
//
/// /    @Test /    @DisplayName("정상적인 회원가입 시 모든 외부 의존 호출이 발생하고 User가 반환된다") /    void createUser_success() { /
/// // given /        UserCreateRequest request = new UserCreateRequest( /                "testuser", "password", "홍길동",
/// "test@email.com", "01012345678", LocalDate.of(1990, 1, 1) /        ); / /
/// given(userRepository.existsByUserLoginId("testuser")).willReturn(false); /
/// given(userGradeRepository.findByGradeName("Standard")).willReturn(mock(UserGrade.class)); / /        User savedUser
/// = User.standard() /                .userId(1L) /                .username("testuser") /
/// .password("password") /                .name("홍길동") /                .email("test@email.com") /
/// .phone("01012345678") /                .birth(LocalDate.of(1990, 1, 1)) /
/// .userGrade(mock(UserGrade.class)) /                .build(); /        given(userRepository.save(any(User.class))) /
///               .willAnswer(invocation -> { /                    User user = invocation.getArgument(0); /
///       Field field = User.class.getDeclaredField("userId"); /                    field.setAccessible(true); /
///            field.set(user, 1L); /                    return user; /                }); / /        // when /
/// User result = userService.createUser(request); / /        // then /
/// assertThat(result.getUsername()).isEqualTo("testuser"); /        verify(cartClient).createCart(); /
